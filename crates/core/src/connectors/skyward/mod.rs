...
use self::mapper::SkywardMapper;

/// Skyward SIS connector implementing the OneRoster 1.1 API.
pub struct SkywardConnector {
    client: OneRosterClient,
}

impl SkywardConnector {
    pub fn new(config: &SisConfig) -> Result<Self> {
        let token_url = config
            .token_url
            .as_ref()
            .ok_or_else(|| ChalkError::Config("token_url is required for Skyward".into()))?;
...
#[async_trait]
impl SisConnector for SkywardConnector {
    async fn full_sync(&self) -> Result<SyncPayload> {
        info!("Starting full sync from Skyward");
        ...
        let orgs = SkywardMapper::normalize_orgs(raw_orgs);
        ...
        let raw_sessions: Vec<_> = sessions_response
            ...
            .collect();
        let academic_sessions = SkywardMapper::normalize_academic_sessions(raw_sessions);
        ...
        let users = SkywardMapper::normalize_users(raw_users);
        ...
        let raw_courses: Vec<_> = courses_response
            ...
            .collect();
        let courses = SkywardMapper::normalize_courses(raw_courses);
        ...
        let raw_classes: Vec<_> = classes_response
            ...
            .collect();
        let classes = SkywardMapper::normalize_classes(raw_classes);
        ...
        let raw_enrollments: Vec<_> = enrollments_response
            ...
            .collect();
        let enrollments = SkywardMapper::normalize_enrollments(raw_enrollments);
        ...
        let raw_demographics: Vec<_> = demographics_response
            ...
            .collect();
        let demographics = SkywardMapper::normalize_demographics(raw_demographics);
        info!(count = demographics.len(), "Fetched demographics");

        info!("Full sync from Skyward completed successfully");
    ...
    async fn test_connection(&self) -> Result<()> {
        info!("Testing Skyward connection");
        self.client.test_connection().await?;
        info!("Skyward connection test successful");
        Ok(())
    }
