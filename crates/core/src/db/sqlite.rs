...
fn row_to_entra_state(r: &sqlx::sqlite::SqliteRow) -> EntraUserState {
    EntraUserState {
        user_sourced_id: r.get("user_sourced_id"),
...
fn row_to_entra_run(r: &sqlx::sqlite::SqliteRow) -> EntraSyncRun {
    EntraSyncRun {
        id: r.get("id"),
        started_at: parse_datetime(r.get("started_at")),
        completed_at: r
...
#[async_trait]
impl EntraSyncStateRepository for SqliteRepository {
    ...
    async fn get_entra_sync_state(&self, user_sourced_id: &str) -> Result<Option<EntraUserState>> {
        let row = sqlx::query("SELECT * FROM entra_sync_state WHERE user_sourced_id = ?1")
            ...
            .fetch_optional(&self.pool)
            .await?;
        Ok(row.as_ref().map(row_to_entra_state))
    }
    ...
    async fn list_entra_sync_states(&self) -> Result<Vec<EntraUserState>> {
        let rows = sqlx::query("SELECT * FROM entra_sync_state ORDER BY user_sourced_id")
            .fetch_all(&self.pool)
            .await?;
        Ok(rows.iter().map(row_to_entra_state).collect())
    }
...
#[async_trait]
impl EntraSyncRunRepository for SqliteRepository {
    ...
    async fn get_latest_entra_sync_run(&self) -> Result<Option<EntraSyncRun>> {
        let row = sqlx::query("SELECT * FROM entra_sync_runs ORDER BY started_at DESC LIMIT 1")
            .fetch_optional(&self.pool)
            .await?;
        Ok(row.as_ref().map(row_to_entra_run))
    }
    ...
    async fn list_entra_sync_runs(&self, limit: i64) -> Result<Vec<EntraSyncRun>> {
        let rows = sqlx::query("SELECT * FROM entra_sync_runs ORDER BY started_at DESC LIMIT ?1")
            ...
            .fetch_all(&self.pool)
            .await?;
        Ok(rows.iter().map(row_to_entra_run).collect())
    }
}
...
fn asset_report_from_row(r: &sqlx::sqlite::SqliteRow) -> AssetReport {
    AssetReport {
        id: r.get("id"),
        name: r.get("name"),
        query: r.get("query"),
...
#[async_trait]
impl AssetReportRepository for SqliteRepository {
    async fn create_asset_report(&self, report: &AssetReport) -> Result<()> {
        ...
        .bind(&report.actor)
        .bind(datetime_to_str(&report.created_at))
        .execute(&self.pool)
        .await?;
        Ok(())
    }
    ...
    async fn get_asset_report(&self, id: &str) -> Result<Option<AssetReport>> {
        let row = sqlx::query("SELECT * FROM asset_reports WHERE id = ?1")
            ...
            .fetch_optional(&self.pool)
            .await?;
        Ok(row.as_ref().map(asset_report_from_row))
    }
    ...
    async fn list_asset_reports(&self) -> Result<Vec<AssetReport>> {
        let rows = sqlx::query("SELECT * FROM asset_reports ORDER BY name")
            .fetch_all(&self.pool)
            .await?;
        Ok(rows.iter().map(asset_report_from_row).collect())
    }
...
fn repair_from_row(r: &sqlx::sqlite::SqliteRow) -> RepairRecord {
    RepairRecord {
        id: r.get("id"),
        asset_id: r.get("asset_id"),
        ticket_id: r.get("ticket_id"),
        ...
        vendor: r.get("vendor"),
        opened_at: parse_datetime(&r.get::<String, _>("opened_at")),
        closed_at: r
...
impl RepairRepository for SqliteRepository {
    async fn create_repair(&self, record: &RepairRecord) -> Result<()> {
        ...
        .bind(&record.vendor)
        .bind(datetime_to_str(&record.opened_at))
        .bind(record.closed_at.as_ref().map(datetime_to_str))
    ...
    async fn open_repair_for_asset(&self, asset_id: &str) -> Result<Option<RepairRecord>> {
        ...
        .fetch_optional(&self.pool)
        .await?;
        Ok(row.map(|r| repair_from_row(&r)))
    }
    ...
    async fn repair_history_for_asset(&self, asset_id: &str) -> Result<Vec<RepairRecord>> {
        ...
        .fetch_all(&self.pool)
        .await?;
        Ok(rows.iter().map(repair_from_row).collect())
    }
}
