...
fn item_from_pg_row(r: &sqlx::postgres::PgRow) -> Item {
    Item {
        id: r.get("id"),
        name: r.get("name"),
...
fn holding_from_pg_row(r: &sqlx::postgres::PgRow) -> ItemHolding {
    ItemHolding {
        id: r.get("id"),
...
impl ItemRepository for PostgresRepository {
    ...
    async fn get_item(&self, id: &str) -> Result<Option<Item>> {
        let row = sqlx::query("SELECT * FROM items WHERE id = $1")
            ...
            .fetch_optional(&self.pool)
            .await?;
        Ok(row.as_ref().map(item_from_pg_row))
    }
    ...
    async fn list_all_items(&self) -> Result<Vec<Item>> {
        let rows = sqlx::query("SELECT * FROM items ORDER BY name")
            .fetch_all(&self.pool)
            .await?;
        Ok(rows.iter().map(item_from_pg_row).collect())
    }
    ...
    async fn create_holding(&self, holding: &ItemHolding) -> Result<()> {
        ...
        .bind(holding.issued_at)
        .bind(holding.returned_at)
        .bind(&holding.actor)
        .execute(&self.pool)
        .await?;
        Ok(())
    ...
    async fn list_holdings_for_item(&self, item_id: &str) -> Result<Vec<ItemHolding>> {
        let rows =
            sqlx::query("SELECT * FROM item_holdings WHERE item_id = $1 ORDER BY issued_at DESC")
                ...
                .fetch_all(&self.pool)
                .await?;
        Ok(rows.iter().map(holding_from_pg_row).collect())
    }
...
fn kb_from_pg_row(r: &PgRow) -> KbArticle {
    KbArticle {
        id: r.get("id"),
        title: r.get("title"),
        body: r.get("body"),
...
#[async_trait]
impl KbRepository for PostgresRepository {
    ...
    async fn list_kb_articles(&self, published_only: bool) -> Result<Vec<KbArticle>> {
        let sql = if published_only {
            "SELECT id, title, body, published, created_at, updated_at FROM kb_articles \
             WHERE published ORDER BY updated_at DESC, id ASC"
        } else {
            "SELECT id, title, body, published, created_at, updated_at FROM kb_articles \
...
#[async_trait]
impl RoutingRuleRepository for PostgresRepository {
...
fn csat_from_pg_row(r: &PgRow) -> CsatResponse {
    CsatResponse {
        id: r.get("id"),
        ticket_id: r.get("ticket_id"),
        token: r.get("token"),
...
#[async_trait]
impl CsatRepository for PostgresRepository {
    async fn create_csat(&self, csat: &CsatResponse) -> Result<bool> {
        let result = sqlx::query(
            "INSERT INTO csat_responses (id, ticket_id, token, score, sent_at) \
             VALUES ($1, $2, $3, $4, $5) ON CONFLICT (ticket_id) DO NOTHING",
        )
        .bind(&csat.id)
        .bind(&csat.ticket_id)
        .bind(&csat.token)
    ...
    async fn get_csat_by_token(&self, token: &str) -> Result<Option<CsatResponse>> {
        ...
        .fetch_optional(&self.pool)
        .await?;
        Ok(row.map(|r| csat_from_pg_row(&r)))
    }
    ...
    async fn csat_stats(&self) -> Result<CsatStats> {
        let row = sqlx::query(
            "SELECT COUNT(*) AS sent, COUNT(score) AS responded, AVG(score)::float8 AS average \
             FROM csat_responses",
        )
        .fetch_one(&self.pool)
        .await?;
